...
class VectorDB:
    def __init__(self, config_dict: dict):
        ...
        if config_dict['embed_fn'] == 'openai':
            ...
            self.embed_fn = embedding_functions.SentenceTransformerEmbeddingFunction(model_name=config_dict['embed_fn'])
    ...
    def get_or_create_collection(self, name):
        logger.info(f'[{self.name}] get_or_create(): {name}')
    ...
    def query(self, texts: list):
        logger.info(f'[{self.name}] querying database for {len(texts)} texts')
        try:
            ...
            logger.error(f'Failed to query collection: {err}')
            results = []
